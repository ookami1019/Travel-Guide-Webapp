  ...
  return (
    <div className="space-y-4">
      {categories.map((category) => (
        <div key={category.id} className="bg-white border text-xs border-zinc-200 rounded-lg p-3 space-y-3 relative group">
          <div className="flex gap-2 items-center mb-1">
            ...
            <input
              ...
              className="font-bold text-xs bg-transparent border-b border-transparent hover:border-zinc-300 focus:border-indigo-500 outline-none px-1 py-0.5"
            />
            <button
              onClick={() => removeCategory(category.id)}
              className="ml-auto p-1 hover:bg-red-50 hover:text-red-500 rounded text-zinc-300 transition-colors"
            >
              <Trash2 size={12} />
            </button>
          </div>
          ...
          <div className="space-y-2 pl-6">
            {category.items.map(item => (
              <div key={item.id} className="flex items-center gap-2 group/item">
                ...
                <input
                  ...
                  className="flex-1 bg-zinc-50 border border-zinc-200 rounded px-2 py-1 text-xs focus:ring-1 focus:ring-indigo-500 outline-none"
                />
                <button
                  onClick={() => removeItem(category.id, item.id)}
                  className="p-1 opacity-0 group-hover/item:opacity-100 hover:text-red-500 text-zinc-400 transition-all rounded"
                >
                  <Trash2 size={12} />
                </button>
              </div>
            ...
            <button
              ...
              className="text-[10px] font-bold text-indigo-500 hover:text-indigo-600 flex items-center gap-1 mt-1"
            >
      ...
      <button
        onClick={addCategory}
        className="w-full py-2 border-2 border-dashed border-zinc-200 rounded-lg text-zinc-500 text-xs font-bold flex items-center justify-center gap-2 hover:bg-zinc-50 hover:border-zinc-300 transition-all"
      >
